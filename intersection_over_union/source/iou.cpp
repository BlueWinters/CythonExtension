    ...
    bool include_edge)
{
    ...
    dtype inter_area = inter_width * inter_height;
    dtype area_a = (x2 - x1 + value) * (y2 - y1 + value);
    dtype area_b = (x4 - x3 + value) * (y4 - y3 + value);

    dtype union_area = area_a + area_b - inter_area;
    if (union_area <= 0) {
        iou = 0;
        return;
    }
...
#if __cplusplus >= 201703L
    if constexpr (std::is_same_v<dtype_iou, int>) {
        // int
        iou = static_cast<float>(static_cast<float>(inter_area) / static_cast<float>(union_area));
    } else {
        // float or double
        iou = inter_area / union_area;
    }
#else
    // auto use reload-function
    iou = divide_inline(&inter_area, &union_area);
#endif
}
...
{
    float iou = 0.f;
    calculateIOU_Template<int, float>(
        x1, y1, x2, y2, x3, y3, x4, y4, iou, include_edge);
    return iou;
}
...
{
    float iou = 0.f;
    calculateIOU_Template<float, float>(
        x1, y1, x2, y2, x3, y3, x4, y4, iou, include_edge);
    return iou;
}
...
{
    double iou = 0.f;
    calculateIOU_Template<double, double>(
        x1, y1, x2, y2, x3, y3, x4, y4, iou, include_edge);
    return iou;
}
    ...
    dtype_iou* iou, bool include_edge)
{
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < M; ++j) {
            ...
            calculateIOU_Template<dtype, dtype_iou>(
                box1[0], box1[1], box1[2], box1[3],
                box2[0], box2[1], box2[2], box2[3],
                iou[i * M + j], include_edge
            );
        }
    }
}
